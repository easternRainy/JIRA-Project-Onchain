...
1. 在Jira的Search Issue中初步筛选，导出csv文件，并放入工作目录中，更名为worklogTest.csv
   ...
   需要更改日期
2. 运行程序，输入需要的column编号， 如果输入000则为默认值0,6,24,27,62
...
8. 最后一列为累计工作时长，只需要关注某工作人员累计时长最后一项即可判断
...
'''start'''
js_HEAD1 = json.dumps(HEAD1, indent=4)
print(js_HEAD1)
print('\n\n')

print('Please input the key of columns you want, split by comma, 000 for default:')
columnsNeed = input()

if columnsNeed == '000':
    columnsNeed = '0,6,24,27,62'

columnsNeed =  columnsNeed.split(',')
...
if is_log_work == 1:
    ...
    df_final = pd.concat([df_final, df_need, df_merge], axis=1)
    ...
    for j in range(1, getRepetition('Log Work') + 1):
        ...
        df_middle = pd.concat([df_middle, df_need, df_merge], axis=1)
    ...
    df_final['Log Work Hour'] = df_final['Log Work Hour'].apply(lambda x: int(x)/3600)
    ...
    # 按Log Work Author排序
    df_final['Log Work Author Upper'] = df_final['Log Work Author'].str.upper()
    df_final = df_final.sort_values('Log Work Author Upper', ascending=1)
    df_final = df_final.drop(columns='Log Work Author Upper')

    # 2019/3/29更新，增加工作时长求和功能，比较粗糙请多谅解。。。存储于最后一列'sum'字段
    df_final['sum']=0
    currentName=''
    sum=0
    i=0
    for head, value in df_final.iterrows():
        # print(value['Log Work Hour'])
        if (value['Log Work Author'] != currentName):
            sum = 0
        sum +=int(value['Log Work Hour'])
        currentName = value['Log Work Author']
        df_final.iloc[i,df_final.shape[1]-1] = sum
        i = i + 1
